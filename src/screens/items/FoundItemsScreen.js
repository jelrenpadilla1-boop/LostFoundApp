...
export default function FoundItemsScreen({ navigation }) {
  ...
  const [statsCards, setStatsCards] = useState([
    { icon: 'box', label: 'Total Items', value: 0, color: '#e50914' },
    { icon: 'check-circle', label: 'Claimed', value: 0, color: '#2e7d32' },
    { icon: 'clock', label: 'Pending', value: 0, color: '#f5c518' },
    { icon: 'trending-up', label: 'This Week', value: 0, color: '#9c27b0' },
  ]);
  ...
  const loadItems = async () => {
    try {
      ...
      if (response.data) {
        ...
        } else if (response.data.foundItems) {
      ...
      if (activeTab === 'recent') {
        ...
        filteredItems = itemsData.filter(item => item.status === 'claimed');
      }
      
      // FIXED: Filter out pending and rejected items for display
      // The backend already handles this, but we do it again to be safe
      ...
      // Calculate stats based on visible items (excluding pending and rejected)
      const visibleItems = filteredItems;
      const totalItems = visibleItems.length;
      const claimedCount = visibleItems.filter(item => item.status === 'claimed').length;
      
      // Calculate this week count for visible items
      const oneWeekAgo = new Date();
      oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
      const thisWeekCount = visibleItems.filter(item => {
        const itemDate = new Date(item.created_at);
        return itemDate >= oneWeekAgo;
      }).length;
      
      // For admins, show pending count from pendingItems
      const pendingCount = isAdmin ? pendingItemsData.length : 0;
      
      setStatsCards([
        { icon: 'box', label: 'Total Items', value: totalItems, color: '#e50914' },
        { icon: 'check-circle', label: 'Claimed', value: claimedCount, color: '#2e7d32' },
        { icon: 'clock', label: 'Pending', value: pendingCount, color: '#f5c518' },
        { icon: 'trending-up', label: 'This Week', value: thisWeekCount, color: '#9c27b0' },
      ]);
  ...
  const renderHeader = () => (
    ...
    >
      <View style={styles.headerTop}>
        <View>
          <Text style={[styles.welcomeText, { fontSize: isTablet ? 34 : 28 }]}>Found Items</Text>
          <Text style={[styles.subText, { fontSize: isTablet ? 15 : 14 }]}>Help return items to their owners</Text>
        </View>
      ...
      <View style={styles.tabsContainer}>
        ...
        <TouchableOpacity style={[styles.tab, activeTab === 'recent' && styles.tabActive, { backgroundColor: isDark ? '#1a1a1a' : '#ffffff', borderColor: isDark ? '#333333' : '#e0e0e0' }]} onPress={() => setActiveTab('recent')}>
          <Feather name="clock" size={16} color={activeTab === 'recent' ? '#e50914' : (isDark ? '#b3b3b3' : '#94a3b8')} />
          <Text style={[styles.tabText, activeTab === 'recent' && styles.tabTextActive, { color: activeTab === 'recent' ? '#e50914' : (isDark ? '#b3b3b3' : '#94a3b8') }]}>Recent</Text>
        </TouchableOpacity>
        
        <TouchableOpacity style={[styles.tab, activeTab === 'claimed' && styles.tabActive, { backgroundColor: isDark ? '#1a1a1a' : '#ffffff', borderColor: isDark ? '#333333' : '#e0e0e0' }]} onPress={() => setActiveTab('claimed')}>
          <Feather name="check-circle" size={16} color={activeTab === 'claimed' ? '#e50914' : (isDark ? '#b3b3b3' : '#94a3b8')} />
          <Text style={[styles.tabText, activeTab === 'claimed' && styles.tabTextActive, { color: activeTab === 'claimed' ? '#e50914' : (isDark ? '#b3b3b3' : '#94a3b8') }]}>Claimed</Text>
        </TouchableOpacity>
    ...
    >
      <View style={[styles.gridCard, { backgroundColor: isDark ? '#1a1a1a' : '#ffffff', borderColor: isDark ? '#333333' : '#e0e0e0' }]}>
        <View style={styles.gridImageContainer}>
          ...
          {/* FIXED: Show badge for ALL items including approved */}
          {item.status && (
        ...
        <View style={styles.gridContent}>
          ...
          <View style={styles.gridFooter}>
            <View style={styles.gridLocation}>
              ...
              <Text style={[styles.gridLocationText, { color: isDark ? '#b3b3b3' : '#94a3b8' }]} numberOfLines={1}>{item.found_location || 'Unknown'}</Text>
            </View>
      ...
      <View style={styles.listContent}>
        ...
        <View style={styles.listMeta}>
          <View style={styles.listLocation}>
            <Feather name="map-pin" size={10} color={isDark ? '#666666' : '#94a3b8'} />
            <Text style={[styles.listLocationText, { color: isDark ? '#b3b3b3' : '#94a3b8' }]} numberOfLines={1}>{item.found_location || 'Unknown'}</Text>
          </View>
    ...
    <>
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: isDark ? '#141414' : '#f8fafc' }]} edges={['top']}>
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <LinearGradient colors={[isDark ? '#1a1a1a' : '#ffffff', isDark ? '#1a1a1a' : '#ffffff']} style={styles.emptyGradient}>
                ...
                <Text style={[styles.emptyTitle, { color: isDark ? '#ffffff' : '#0f172a' }]}>No found items found</Text>
                <Text style={[styles.emptySubtitle, { color: isDark ? '#b3b3b3' : '#64748b' }]}>
                  {searchQuery ? 'Try a different search term' : 'Be the first to report a found item'}
                </Text>
        ...
        <Animated.View style={[styles.floatingButton, { transform: [{ scale: floatingButtonAnim }] }]}>
          <TouchableOpacity onPress={() => navigation.navigate('CreateItem', { type: 'found' })} activeOpacity={0.9}>
